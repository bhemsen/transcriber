...
impl Session {
    ...
    /// and blocks until each has, so no thread is still writing once this
    /// returns.
    ///
    /// # Errors
    ///
    ...
    pub fn request_stop(&mut self) -> Result<(), SessionError> {
        ...
        self.remote.request_stop_and_join()?;
        if let Some(local) = self.local.as_mut() {
            local.request_stop_and_join()?;
        }
...
mod tests {
    ...
    use transcriber_core::{AttestationVersion, ConsentAttestation, SessionState};
